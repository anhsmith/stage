...
distribution for \(m_{50}\), representing the value of \(x\) at which the probabilities \(\Pr(y=0)\) and \(\Pr(y=1)\) are equal.
...
U    <- 1700
sigma <- 120
m50  <- 1350          # transition point (direct sampling parameter)
d    <- 300            # separation
mu0  <- m50 - d/2      # 1200
mu1  <- m50 + d/2      # 1500
...
The **transition point** \(m_{50}\) is the value of \(x\) where the probability of
observing either class is equal:
...
By allowing different behaviours on either side of the transition, the model can capture
...
Because \(d\) and `sigma_x` are constrained to be positive in the Stan model,
their Normal priors are effectively **half-Normal** distributions (the prior mass
below zero is folded onto the positive half).
...
**Why \(m_{50}\) is the midpoint between \(\mu_0\) and \(\mu_1\).**
In the transition region (\(\mu_0 < x < \mu_1\)), both unnormalised densities
take Gaussian form:

\[
\tilde f_0(x) = \exp\!\left(-\frac{(x - \mu_0)^2}{2\sigma^2}\right), \qquad
\tilde f_1(x) = \exp\!\left(-\frac{(x - \mu_1)^2}{2\sigma^2}\right).
\]

Since both Gaussians share the same \(\sigma\), the unnormalised densities are
equal at the arithmetic midpoint \(x^* = (\mu_0 + \mu_1)/2 = m_{50}\). With equal
class priors, the normalising constants \(C_0\) and \(C_1\) cancel in the
likelihood ratio, so the 50% crossing point is exactly \(m_{50}\) regardless of
the survey bounds \(L\) and \(U\). This is the key invariance property: changing
the sampling domain does not shift the estimated transition point.
...
- The single-population model directly samples `m50` (\(m_{50}\)), `d`, and
  `sigma_x` (\(\sigma\)). The transition point is a primary sampling parameter,
  not a derived quantity.
- The multi-population (hierarchical) model introduces population-specific
  transition points via a random-effects structure:
  \[
  \texttt{m50\_pop}[j] = m_{50} + z_j \, \sigma_\alpha,
  \]
  where \(m_{50}\) is the global mean transition point, \(z_j\) are
  standard-normal effects, and \(\sigma_\alpha\) is the population-level standard
  deviation.
